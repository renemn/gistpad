...
const compileMainJS = (name = 'main') => {
  const src = config.paths.main.js;
  return compileJS(src).then(({ code, cache }) => ({
    context: name, name, type: 'js', src, code, cache
  }));
};

const compileRendererJS = () => config.renderers.map(name => {
  const src = config.paths.renderers[name].js;
  return compileJS(src).then(({ code, cache }) => ({
    context: 'renderer', name, type: 'js', src, code, cache
  }));
});

const compileRendererCSS = () => config.renderers.map(name => {
  const src = config.paths.renderers[name].css;
  return compileCSS(src).then(({ css }) => ({
    context: 'renderer', name, type: 'css', src, code: css
  }));
});
...
module.exports = () => (
  new Promise((resolve, reject) => {
    ...
    Promise
      .all([].concat.apply([], [compileRendererCSS(), compileRendererJS(), compileMainJS()]))
      .then((allBundles) => {
